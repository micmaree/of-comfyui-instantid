...
RUN pip install --no-cache-dir     "diffusers>=0.36,<0.40"     "transformers>=4.46,<5"     "huggingface_hub>=0.25,<1.0"     "accelerate>=0.34"     "safetensors>=0.4"     "peft>=0.17,<0.20"     "bitsandbytes>=0.43"     "torchao>=0.6,<0.9"     prodigyopt hf_transfer sentencepiece tensorboard     pyyaml oyaml omegaconf toml python-dotenv python-slugify     pillow opencv-python-headless einops kornia     lycoris-lora "open_clip_torch>=2.20,<3" "timm<2" "k-diffusion>=0.1"     flatten-json controlnet-aux
